...
if grep -q "patch" "${SCRIPT_DIR}/Dockerfile"; then
    echo "✅ Dockerfile 包含必要的系统依赖（patch）"
else
...
# 7. 验证镜像中的 qdrant-client
echo "7. 验证镜像中的 qdrant-client 安装..."
if docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python -c "from qdrant_client import QdrantClient; print('✅ qdrant-client 已正确安装')" 2>/dev/null; then
    echo "✅ 验证成功：qdrant-client 已正确安装"
else
    echo "❌ 验证失败：qdrant-client 未正确安装"
    echo ""
    echo "检查已安装的包："
    docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" pip list | grep qdrant || echo "未找到 qdrant-client"
    exit 1
fi
